...
const STATUS_OPTIONS = [
  ...
  },
  {
    ...
    border: "#bfdbfe",
    accent: "#60a5fa",
    rowBg: "linear-gradient(180deg, #ffffff 0%, #f5f9ff 100%)",
  },
  {
    value: "done",
    label: "Hoàn thành",
...
const MANAGER_REVIEW_OPTIONS = [
  { value: "dong_y_phe_duyet", label: "Đồng ý phê duyệt" },
  { value: "tra_lai", label: "Trả lại" },
];
  ...
  return (
    <aside style={styles.sidebar}>
      ...
      <div style={styles.sideCard}>
        ...
        <button onClick={onSwitchAccount} style={{ ...styles.topAction, width: "100%", justifyContent: "center", marginBottom: 8 }}>
          <IconSwitch />
          Đổi tài khoản
        </button>

        <button onClick={onLogout} style={{ ...styles.topAction, width: "100%", justifyContent: "center" }}>
          <IconLogout />
          Đăng xuất
        </button>
      </div>
  ...
  return (
    <div style={styles.card}>
      ...
      <div style={{ display: "grid", gap: 8 }}>
        {filtered.length === 0 ? (
          <div style={styles.emptyRow}>Không có công việc trong nhóm tổng quan này.</div>
        ) : (
          filtered.map((w) => {
...
function CreateWorkForm({ onCreated, currentUser }) {
  ...
  const [assignee, setAssignee] = useState(currentUser.role === "staff" ? currentUser.name : "");
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      assignee: (currentUser.role === "staff" ? currentUser.name : assignee.trim()) || "",
      priority,
    ...
    setPriority("medium");
    setDueDate("");
    if (currentUser.role !== "staff") setAssignee("");
  ...
  return (
    <form onSubmit={handleSubmit} style={styles.card}>
      <div style={styles.sectionHeader}>
        <div style={styles.sectionTitleWrap}>
          ...
          <div>
            ...
            <div style={styles.sectionSub}>Khởi tạo công việc mới cho phòng KHTH</div>
          </div>
        </div>
      </div>
      ...
      <div
        style={{
          display: "grid",
          gridTemplateColumns: currentUser.role === "staff"
            ? "2fr 2fr 1fr 1fr auto"
...
function WorkCard({ workItem, onSelect, onMove, onDelete }) {
    ...
    >
      ...
      <div style={styles.workLayout}>
        <div style={styles.workMain}>
          ...
          >
            {workItem.description || "Không có mô tả"}
          </div>
          ...
          <div
            style={{
              ...
              fontWeight: overdue ? 800 : 700,
            }}
          >
            Hạn: {formatDate(workItem.due_date)} {overdue ? "• QUÁ HẠN" : ""}
          </div>
        </div>
...
function WorkflowBoard({ groupedWorks, onSelect, onMove, onDelete }) {
  return (
    <div style={styles.boardCard}>
      <div style={styles.boardHeader}>
        <div>
          <div style={styles.boardTitle}>Quản lý công việc theo luồng trạng thái</div>
          <div style={styles.boardSub}>Hiển thị theo hàng ngang, tối ưu kích thước để nhìn rõ nội dung công việc</div>
        </div>
      </div>
...
function DetailPanel({
  ...
  const [managerDecision, setManagerDecision] = useState("dong_y_phe_duyet");
  ...
  const handleSave = async () => {
    await onSaveDetail(selectedWork.id, form);
  };
    ...
    <div style={styles.detailPanel}>
      ...
      <div style={styles.sectionHeader}>
        <div style={styles.sectionTitleWrap}>
          ...
          <div>
            <div style={styles.sectionTitle}>Chi tiết công việc</div>
            <div style={styles.sectionSub}>Quản lý nội dung, AI và lịch sử thay đổi</div>
          </div>
        </div>
      ...
      {currentUser.role === "manager" && (
        <div style={styles.subCard}>
          ...
          <div style={{ marginBottom: 10 }}>
            <label style={styles.label}>Kết quả xử lý báo cáo</label>
            <select
          ...
          <div style={{ fontSize: 12, color: "#64748b", lineHeight: 1.6 }}>
            Lãnh đạo có thể xử lý các công việc đang đề nghị phê duyệt bằng cách đồng ý phê duyệt hoặc trả lại.
          </div>
        </div>
      )}
...
export default function TaskBoard({ currentUser, onLogout, onSwitchAccount }) {
  ...
  const worksByRole = useMemo(() => {
    if (currentUser.role === "manager") {
      return allWorks.filter((w) => w.status !== "done");
    }
  ...
  const filteredWorks = useMemo(() => {
    return visibleWorks.filter((workItem) => {
      ...
      const matchesSearch =
        !q ||
        workItem.title?.toLowerCase().includes(q) ||
        workItem.description?.toLowerCase().includes(q) ||
        workItem.assignee?.toLowerCase().includes(q);
    ...
    <div style={styles.page}>
      <div style={styles.shell}>
        ...
        <main style={styles.main}>
          <Topbar currentUser={currentUser} onLogout={onLogout} onSwitchAccount={onSwitchAccount} />
          <Stats
            workItems={worksByRole}
            activeOverview={activeOverview}
            setActiveOverview={setActiveOverview}
          />
          <OverviewDashboard workItems={worksByRole} activeOverview={activeOverview} />
          <CreateWorkForm onCreated={fetchWorks} currentUser={currentUser} />
          <FilterBar
            search={search}
            setSearch={setSearch}
